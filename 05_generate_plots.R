...
resultCoeff_long <- resultCoeff %>% 
  ...
  mutate(Coefficient = as.numeric(Coefficient), 
         ...
         Covariate = factor(Covariate, levels = names(resultCoeff)[1:13], labels = 
                              c("HbA1c", "rs4506565 1", "rs4506565 2", "AGE", "SEX TRUE", "INSURANCE TRUE", 
...
range_coef <- list(Covariate == "HbA1c" ~ scale_y_continuous(limits = c(means.coef$mean[1] - 0.15, means.coef$mean[1] + 0.15)),
                   ...
                   Covariate == "SEX TRUE" ~ scale_y_continuous(limits = c(means.coef$mean[5] - 0.2, means.coef$mean[5] + 0.2)),
                   Covariate == "INSURANCE TRUE" ~ scale_y_continuous(limits = c(means.coef$mean[6] - 0.2, means.coef$mean[6] + 0.2)),
                   Covariate == "RACE AFR" ~ scale_y_continuous(limits = c(means.coef$mean[7] - 0.25, means.coef$mean[7] + 0.25)),
                   Covariate == "RACE AMR" ~ scale_y_continuous(limits = c(means.coef$mean[8] - 0.25, means.coef$mean[8] + 0.25)),
                   Covariate == "RACE SAS" ~ scale_y_continuous(limits = c(means.coef$mean[9] - 0.25, means.coef$mean[9] + 0.25)),
                   Covariate == "RACE EAS" ~ scale_y_continuous(limits = c(means.coef$mean[10] - 0.25, means.coef$mean[10] + 0.25)),
                   Covariate == "BMI" ~ scale_y_continuous(limits = c(means.coef$mean[11] - 0.25, means.coef$mean[11] + 0.25)),
                   Covariate == "SMOKE 2" ~ scale_y_continuous(limits = c(means.coef$mean[12] - 0.25, means.coef$mean[12] + 0.25)),
                   Covariate == "SMOKE 3" ~ scale_y_continuous(limits = c(means.coef$mean[13] - 0.25, means.coef$mean[13] + 0.25)))
