...
generatedata <- function(digit){
    ...
    vars_to_delete <- c("ln_na_avg", "ln_k_avg", "ln_kcal_avg", "ln_protein_avg",
                        ...
                        "c_ln_na_avg", "c_ln_k_avg", "c_ln_kcal_avg", "c_ln_protein_avg",
                        "sex", "bkg")
    pop[, (vars_to_delete) := NULL]
    
    
    if(!dir.exists('./data')){system('mkdir ./data')}
    save(pop,file=paste0('./data/', digit, '.RData'),compress = 'xz')
}
