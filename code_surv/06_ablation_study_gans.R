...
show_bias <- function(tpvmi_gans_imp){
...
for (i in 1:100){
  ...
  if (!file.exists(paste0("./simulations/Ablation/Base/", digit, ".RData"))){
    tpvmi_gans_imp <- tpvmi_gans(samp, m = 20, epochs = 10000,
                             data_info = info, params = list(balancebatch = F,
                                                             pac = 1, beta = 0,
                                                             num = "general", cat = "general"),
                             device = "cpu")
    save(tpvmi_gans_imp, file = paste0("./simulations/Ablation/Base/", digit, ".RData"))
    show_bias(tpvmi_gans_imp)
  }
  #### BalSamp
  cat("BalSamp \n")
  if (!file.exists(paste0("./simulations/Ablation/BalSamp/", digit, ".RData"))){
    tpvmi_gans_imp <- tpvmi_gans(samp, m = 20, epochs = 10000,
                             data_info = info, params = list(balancebatch = T,
                                                             pac = 1, beta = 0,
                                                             num = "general", cat = "general"),
                             device = "cpu")
    save(tpvmi_gans_imp, file = paste0("./simulations/Ablation/BalSamp/", digit, ".RData"))
    show_bias(tpvmi_gans_imp)
  }
  #### PacGAN 
  cat("PacGAN \n")
  if (!file.exists(paste0("./simulations/Ablation/PacGAN/", digit, ".RData"))){
    tpvmi_gans_imp <- tpvmi_gans(samp, m = 20, epochs = 10000,
                             data_info = info, params = list(balancebatch = F,
                                                             ...
                                                             num = "general", cat = "general"),
                             device = "cpu")
    save(tpvmi_gans_imp, file = paste0("./simulations/Ablation/PacGAN/", digit, ".RData"))
    show_bias(tpvmi_gans_imp)
  }
  cat("CE Loss \n")
  ...
  if (!file.exists(paste0("./simulations/Ablation/CELoss/", digit, ".RData"))){
    tpvmi_gans_imp <- tpvmi_gans(samp, m = 20, epochs = 10000,
                             data_info = info, params = list(balancebatch = F,
                                                             ...
                                                             num = "general", cat = "general"),
                             device = "cpu")
    save(tpvmi_gans_imp, file = paste0("./simulations/Ablation/CELoss/", digit, ".RData"))
    show_bias(tpvmi_gans_imp)
  }
  cat("Cat Proj \n")
  ...
  if (!file.exists(paste0("./simulations/Ablation/CatProj/", digit, ".RData"))){
    tpvmi_gans_imp <- tpvmi_gans(samp, m = 20, epochs = 10000,
                             ...
                             device = "cpu")
    save(tpvmi_gans_imp, file = paste0("./simulations/Ablation/CatProj/", digit, ".RData"))
    show_bias(tpvmi_gans_imp)
  }
  cat("MMER \n")
  ...
  if (!file.exists(paste0("./simulations/Ablation/MMER/", digit, ".RData"))){
    tpvmi_gans_imp <- tpvmi_gans(samp, m = 20, epochs = 10000,
                             ...
                             device = "cpu")
    save(tpvmi_gans_imp, file = paste0("./simulations/Ablation/MMER/", digit, ".RData"))
    show_bias(tpvmi_gans_imp)
  }
}
