samplebatches <- function(data_original, data_training, tensor_list, 
                          ...
                          num_vars, new_col_names, weight_var, batch_size, at_least_p = 0.2){
...
# samplebatches <- function(data_original, data_training, tensor_list, phase1_rows, phase2_rows, 
#                           phase1_vars, phase2_vars, num_vars, new_col_names, weight_var, batch_size, at_least_p = 0.2){
#   #Provide a case-control based depending on phase1 and phase2 logical variables
#   cat_names <- unlist(new_col_names)
#   phase1_bins <- cat_names[!(cat_names %in% phase2_vars)] 
#   phase2_bins <- phase2_vars[!(phase2_vars %in% num_vars)]
#   
#   #phase1_bins <- phase1_bins[sapply(phase1_bins, function(i){length(unique(data_training[phase1_rows, i])) != 1})]
#   #phase2_bins <- phase1_bins[sapply(phase1_bins, function(i){length(unique(data_training[phase2_rows, i])) != 1})]
#   
...
#     }
#   }else if (length(phase1_bins) > 0){
#     curr_col <- sample(phase1_bins, 1)
#     referring_1 <- data_training[phase1_rows, curr_col]
#     referring_2 <- data_training[phase2_rows, curr_col]
#     unicats <- unique(referring_1)
#     n_unicats <- length(unicats)
#     
#     for (cat in 1:n_unicats){
...
#     }
#   }else{
#     sampphase1 <- sample(phase1_rows, size = sum(n1))
#     sampphase2 <- sample(phase2_rows, size = sum(n2))
#     sampled_ind <- c(sampled_ind, sampphase1, sampphase2)
#   }
#   batches <- lapply(tensor_list, function(tensor) tensor[sampled_ind, , drop = FALSE])
#   return (batches)
