generator.mlp <- torch::nn_module(
  ...
  initialize = function(params, ncols, nphase2, nphase1, rate, ...){
    ...
    for (i in 1:length(params$g_dim)){
      self$seq$add_module(paste0("Residual_", i), Residual(dim1, params$g_dim[i], rate))
    ...
    if (params$component == "match_p1"){
      ...
      for (i in 1:length(params$g_dim)){
        self$seqA$add_module(paste0("Residual_", i), Residual(dim1, params$g_dim[i], rate))
        dim1 <- dim1 + params$g_dim[i]
      }
  ...
  forward = function(input, ...){
    conditions <- self$dropout(input[, (self$params$noise_dim + 1):self$dim1])
    input <- torch_cat(list(input[, 1:self$params$noise_dim], conditions), dim = 2)
