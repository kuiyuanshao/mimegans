...
mimegans <- function(data, m = 5, 
  ...
  phase2_bins <- cat_vars[cat_vars %in% phase2_vars]
  data_original <- data
  if (params$num == "mmer"){
  ...
  for (i in 1:epochs){
    ...
    for (d in 1:discriminator_steps){
      batch <- sampleBatch(data, tensor_list, phase1_bins, phase2_bins,
...
mimegans.proj <- function(data, m = 5, 
  ...
  allcats_p1 <- lapply(bins_by_enc(phase1_vars_encode), 
  ...
  #                        args = list(params, ncols - length(conditions_vars_encode), 
  ...
  for (i in 1:epochs){
    ...
    for (d in 1:discriminator_steps){
      ...
      fake <- gnet(fakez_AC, allcats_p2)
      X_fake <- fake[[1]]
      A_fake <- fake[[2]]
      X_fake <- fake[[3]]# activationFun(X_fake, allcats_p2, params)

      #A_fake <- gnet_phase1(torch_cat(list(fakez, X_fake), dim = 2))
      ...
      d_loss <- -(torch_mean(x_true_p1) - torch_mean(x_fake_p1))
      if (lambda > 0){
    ...
    X_fake <- fake[[1]]
    A_fake <- fake[[2]]
    X_fakeact <- fake[[3]]# activationFun(X_fake, allcats_p2, params)
    # A_fake <- gnet_phase1(torch_cat(list(fakez, X_fakeact), dim = 2))
    ...
    recon_loss <- recon_loss + params$alpha * mm + (ce / length(allcats_p1))
    g_loss <- adv_term + recon_loss
    ...
    # 
    # A_fakeact <- activationFun(A_fake, allcats_p1, params)
    # fake_AC_p1 <- torch_cat(list(A_fakeact, A, C), dim = 2)
    # x_fake_p1 <- -torch_mean(dnet_phase1(fake_AC_p1))
    # 
    # mm <- nnf_mse_loss(A_fakeact[, num_inds_p1, drop = F], A[, num_inds_p1, drop = F])
    # ce <- torch_tensor(0, device = device)
    # for (cat in allcats_p1){
    #   ce <- ce + nnf_cross_entropy(A_fakeact[, cat, drop = F], 
    #                                torch_argmax(A[, cat, drop = F], dim = 2), 
    #                                reduction = "mean")
    # }
    # g_p1_loss <- x_fake_p1 + mm + ce / length(allcats_p1)
...
mimegans.full <- function(data, m = 5, 
                          num.normalizing = "mode", cat.encoding = "onehot", 
                          device = "cpu", epochs = 10000, 
                          params = list(), data_info = list(), 
                          save.step = NULL){
  ...
  for (i in 1:epochs){
    ...
    #                                reduction = "mean")
    # }
    F_ <- torch_cat(list(X, A, C), dim = 2)
    curr_var <- batch[[7]]
    curr_ind_p1 <- bi[[curr_var[1]]]
    curr_ind_p2 <- bi[[curr_var[2]]]
    ce <- nnf_cross_entropy(F_fake[I$logical_not(), curr_ind_p1, drop = F],
                            torch_argmax(F_[I$logical_not(), curr_ind_p1, drop = F], dim = 2),
                            reduction = "mean") +
      nnf_cross_entropy(F_fake[I, curr_ind_p2, drop = F],
                        torch_argmax(F_[I, curr_ind_p2, drop = F], dim = 2),
                        reduction = "mean")
