...
generateImpute <- function(gnet_list, m = 5, data_original, 
  ...
  M_t <- tensor_list[[1]]; C_t <- tensor_list[[2]]; X_t <- tensor_list[[3]]; A_t <- tensor_list[[4]]
  ...
  process_generated_batch <- function(gen_raw, A_curr, C_curr, rows_idx = NULL) {
    ...
    df_final <- do.call(denormalize, args = list(data = df_decoded, num_vars = data_info$num_vars, norm_obj = data_norm))$data
  ...
  for (z in 1:m){
    ...
    curr_gsample <- do.call(denormalize, args = list(data = curr_gsample, num_vars = data_info$num_vars, norm_obj = data_norm))
    gsamples <- curr_gsample$data[, names(data_original)]
    if (params$num == "mmer"){
      gsamples[, num_inds_gen] <- data_original[, num_inds_ori] - gsamples[, num_inds_gen]
    }
    
    # --- Rejection Sampling & PMM (Unchanged) ---
    ...
    for (v in names(lb)){
      ...
      if (length(oob_ind) > 0){
        yhatmis <- pmm(gsamples[[v]][data_original$R == 1], gsamples[[v]][oob_ind], data_original[[v]][data_original$R == 1], 5)
    ...
    if (any(num_col)) {
      out_num <- data_mask[, num_col] * as.matrix(data_original[, num_col]) + (1 - data_mask[, num_col]) * as.numeric(as.matrix(gsamples[, num_col]))
