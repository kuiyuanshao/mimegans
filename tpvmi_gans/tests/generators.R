...
generator.fullmlp <- torch::nn_module(
  "Generator",
  initialize = function(params, ncols, nphase2, rate, nphase1, ...){
    self$params <- params
  ...
  forward = function(input, ...){
    ...
    output_X <- output[, 1:self$nphase2]
    return (list(output_X, output))
  }
)

generator.multimlp <- torch::nn_module(
  "Generator",
  initialize = function(params, ncols, nphase2, rate, nphase1, ...){
    self$params <- params
    dim1 <- params$noise_dim + ncols - nphase2
    ...
    self$seqX <- torch::nn_sequential(nn_dropout(0.25))
    for (i in 1:params$n_g_layers){
      self$seqX$add_module(paste0("Residual_", i), Residual(dim1, dim2, rate))
      dim1 <- dim1 + dim2
    }
    self$seqX$add_module("Linear", nn_linear(dim1, nphase2))
  ...
  forward = function(input, allcats_p2, ...){
    output_X <- self$seqX(input)
    input_X <- activationFun(torch_clone(output_X), 
                             allcats_p2, self$params)
    output_A <- self$seqA(input_X)
    # output_A <- output_X + output_A
    return (list(output_X, output_A, input_X))
  }
)
