...
cwgangp_default <- function(batch_size = 500, lambda = 10, alpha = 0, beta = 1, proj_weight = 1, at_least_p = 1, g_dropout = 0,
                            lr_g = 2e-4, lr_d = 2e-4, g_betas = c(0.5, 0.9), d_betas = c(0.5, 0.9),
                            noise_dim = 64, cond_dim = 128, g_dim = c(256, 256), d_dim = c(256, 256), 
                            pac = 2, discriminator_steps = 1, tau = 0.2, hard = F, type_g = "mlp", type_d = "mlp",
...
tpvmi_gans <- function(data, m = 5, 
  ...
  for (model_idx in 1:num_models_to_train) {
    ...
    if (params$mi_approx != "SWAG"){
      ...
      # Assumes 25 times larger learning rate
        ...
        if (!is_swa_phase) {
          ...
          for (key in names(curr_state)) {
            ...
            if (inherits(val, "torch_tensor") && val$dtype == torch_float()) {
              ...
              if (swa_n == 1) {
                swa_mean[[key]] <- p_data; swa_sq_mean[[key]] <- p_data^2; swa_cov_mat[[key]] <- list() 
              } else {
                ...
                q <- swa_cov_mat[[key]]; q[[length(q) + 1]] <- dev
                if (length(q) > max_num_models) q <- q[-1] 
                swa_cov_mat[[key]] <- q
              }
            } else { swa_mean[[key]] <- val$detach()$clone() }
          }
        }
      }
    ...
    training_loss_list[[model_idx]] <- data.frame(training_loss)
  ...
  result <- generateImpute(gnet_list, m = m, data_original, 
                           data_info, data_norm, data_encode, data_training,
                           phase1_vars_encode, phase2_vars_encode,
                           num.normalizing, cat.encoding, device, params, 
                           cat_groups_p2, num_inds_p2, tensor_list,
                           swag_stats)
  out <- list(imputation = result$imputation, gsample = result$gsample, loss = training_loss_list)
  return (out)
}